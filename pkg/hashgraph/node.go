...
type Node struct {
    ...
    seeDPMemory                   map[*Event]map[*Event]bool
}
...
func NewNode(initialHashgraph map[string][]*Event, address string) *Node {
    return &Node{
        ...
        seeDPMemory:                   make(map[*Event]map[*Event]bool),
    }
}
...
func (n *Node) see(current *Event, target *Event) bool {
    dpMap, ok := n.seeDPMemory[current]

    if !ok {
        dpMap := make(map[*Event]bool)
        n.seeDPMemory[current] = dpMap
    }

    cachedResult, ok := dpMap[target]
